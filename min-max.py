# In this mission you should write you own py3 implementation (but you
# can use py2 for this) of the built-in functions min and max. Some
...
def min(*args, **kwargs):
    ...
    for element in elements:
        if currmin is None or key(element) < key(currmin):
            currmin = element
    return currmin
...
def max(*args, **kwargs):
    ...
    currmax = None
    for element in elements:
        if currmax is None or key(element) > key(currmax):
            currmax = element
    return currmax
