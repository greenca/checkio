...
# all the original letters exactly once. Two words are anagrams to 
# each other if we can get one from another by rearranging the letters. 
...
def verify_anagrams(first_word, second_word):
    word1 = sorted(first_word.lower().replace(' ', ''))
    word2 = sorted(second_word.lower().replace(' ', ''))
    return word1 == word2
